import { Clear, QrCodeScanner, Scanner } from "@mui/icons-material"
...
interface ISendTo {
    onClick: () => void
    transferToken: (amount: number, address: string) => Promise<void>
    busdBal?: number,
    isLoading: boolean
}


const SendTo = ({onClick, transferToken, busdBal, isLoading}: ISendTo) => {
  ...
  return (
        <form onSubmit={ handleSubmit } className="flex flex-col justify-between items-center space-y-6 md:bg-cardBackground mt-4 rounded-md md:shadow-md px-4 py-4 w-96 h-96">
          ...
          <div className="flex flex-row justify-between items-center w-full">
           ...
           <label className="relative block w-full">
            ...
            <span className="absolute inset-y-0 right-0 flex items-center pr-3">
                {
                    isTyping ? <button onClick={ clearAddress }><Clear /></button> : <QrCodeScanner style={{color: "white"}} />
                }
            </span>   

            <span className="absolute inset-y-0 right-0 flex items-center pr-3">
                {
                    isTyping ? <button onClick={ clearAddress }> <Clear style={{color: "white"}}/> </button> : <QrCodeScanner style={{color: "white"}} />
                }
            </span>
           </label>
...

          {
             isAddressValid ? (
              <input id="amount" className=" w-full border bg-cardBackground text-white border-slate-400 drop-shadow-md rounded-md py-2 pl-3 pr-4"
                onChange={ onAmountChange } placeholder="Enter amount" type="number" disabled={isLoading}/>
             ) : null
          }

          {
            ...
            <div className="text-center text-white text-sm">
               Max Amount: { busdBal } BUSD
            </div>  
          </div>

          {
            isAddressValid && isAmountValid ? (
              <button disabled={isLoading} type="submit" className={`shadow-lg bg-blue-500 rounded-full text-white py-2 px-4 hover:bg-blue-400 w-full justify-between`}>
                {
                    isLoading ? <svg role="status" className="inline w-5 h-5 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
